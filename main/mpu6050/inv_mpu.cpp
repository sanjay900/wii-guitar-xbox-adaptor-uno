...
int mpu_init(struct int_param_s *int_param)
{
    ...
    if (rev) {
...
#ifdef MPU_DEBUG
            Serial.print("Unsupported software product rev: "); Serial.printf(rev);
#endif
            return -4;
        }
    } else {
        ...
        if (!rev) {
#ifdef MPU_DEBUG
            Serial.printf("Product ID read as 0 indicates device is either incompatible or an MPU3050");
#endif
            return -6;
        } else if (rev == 4) {
#ifdef MPU_DEBUG
            Serial.printf("Half sensitivity part found.");
#endif
            st->chip_cfg.accel_half = 1;
        } else
            st->chip_cfg.accel_half = 0;
...
#ifdef AK89xx_SECONDARY
#ifdef MPU_DEBUG
    Serial.printf("Setting up compass");
#endif
    errCode = setup_compass();
    if (errCode != 0) {
#ifdef MPU_DEBUG
       Serial.print("Setup compass failed: "); Serial.printf(errCode); 
#endif
    }
...
#ifdef MPU_DEBUG
           printf("Firmware compare failed addr "); printf("%d\n",ii);
            for (int i = 0; i < this_write; i++) {
              printf("0x%02x ", progBuffer[i]); 
            }
            printf("\n");
            for (int i = 0; i < this_write; i++) {
              printf("0x%02x ", cur[i]); 
...
static int setup_compass(void)
{
...
#ifdef MPU_DEBUG
        Serial.printf("Compass not found.");
#endif
        return -1;
    }
    ...
    Serial.print(st->chip_cfg.mag_sens_adj[2]); Serial.printf();
#endif
